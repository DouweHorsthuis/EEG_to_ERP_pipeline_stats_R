...
Plot_chans={'chan1' 'chan2'}; %either channel names e.g. {'fpz' 'F1'} or {'All'}
Plot_bins={'bin1' 'bin2'}; %either names from ERP.bindescr or 'All'
epoch_time_x = [startms endms]; %time in ms e.g. [-50 100]
time_points_x = [x_ticks]; %Time point for the plot in ms -> individual times eg. [-50 10 12 200 322] steps [ 0:50:300] combo [-50 0:100:300]
amplitude_y = [min_y max_y]; %min and max amplitudes on plot
amplitude_points_x = [y_ticks]; %Amplitude on y-axis similar to time_point_x
plot_tiles = [N_horizontal_figs N_vertical_figs]; %tiles in the end figures 
%needs no input
